...
class FiltroMotivo implements JsonSerializable
{
	...
	public static function exist_motivo_con_filtro($coneccion, $id_filtro, $id_motivo)
	{
		$is_multiple = 0;
		if ($id_filtro && $id_motivo) {
			$consulta = "select * 
						from filtros_motivos
	...
	public static function get_list_motivo_por_id_filtro($coneccion, $id_filtro)
	{
		$list = [];
		if ($id_filtro) {
			$consulta = "select * 
            ...
            while($res = mysqli_fetch_assoc($ret)) {
                $list[] = new self(
                                   coneccion: $coneccion,
                                   id_filtro_motivo: $res["id_filtro_motivo"]
                                   );
            }
        }
        return $list;
