	...
	if ($filas === 0) {
		...
		while($resultados = mysqli_fetch_array($consulta)) {
			...
			if(isset($_REQUEST["number"])&&$_REQUEST["number"] > 1){
				...
				$mensaje .= '<td><button type = "button" class = "btn btn-outline-success" onClick="seleccionMotivo(\''.$Motivo.'\','.$ID_Motivo.','.$number.')" data-dismiss="modal">seleccionar</button></td>
						    </tr>';
			} else{
				$mensaje .= '<td><button type = "button" class = "btn btn-outline-success" onClick="seleccionMotivo(\''.$Motivo.'\','.$ID_Motivo.',1)" data-dismiss="modal">seleccionar</button></td>
						    </tr>';
			}  
