        ...
        <div class = "row">
          <?php  
            if(isset($_REQUEST["Fecha_Desde"]) && $_REQUEST["Fecha_Desde"]!=null && isset($_REQUEST["Fecha_Hasta"]) && $_REQUEST["Fecha_Hasta"] != null){
              ...
              $ConsultarMovimientos = "select M.id_movimiento, M.fecha, P.apellido, P.nombre, M.motivo_1, M.motivo_2, M.motivo_3, M.observaciones, R.responsable from movimiento M, responsable R, persona P, motivo MT, categoria C where M.id_resp = R.id_resp and M.id_persona = P.id_persona and M.estado = 1 and M.fecha between '$Fecha_Desde' and '$Fecha_Hasta'";
              ...
              $ConsultarMovimientos .= " group by M.id_movimiento order by M.fecha";



              $MensajeErrorMovimientos = "No se pudo consultar los movimientos de la persona";

              $TomarMovimientosEntreFechas = mysqli_query($Con->Conexion,$ConsultarMovimientos) or die($MensajeErrorMovimientos." - ".$ConsultarMovimientos);

              $Rows = mysqli_num_rows($TomarMovimientosEntreFechas);

              if($Rows == 0){
                echo "<div class = 'col'></div>";
                echo "<div class = 'col-6'>";
                echo "<p class = 'TextoSinResultados'>No se encontraron Resultados</p><center><button class = 'btn btn-danger' onClick = 'location.href= \"view_movfechas.php\"'>Atras</button></center>";
                echo "</div>";
                echo "<div class = 'col'></div>";
              }

              while ($RetMovimientos = mysqli_fetch_assoc($TomarMovimientosEntreFechas)) {
