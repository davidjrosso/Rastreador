		...
		if($Regis > 0){
			...
			while ($Ret = mysqli_fetch_array($Con->ResultSet)) {
				...
				$Table .= "<tr>
							 ...
							 <td>" . $Usuario . "</td>
							 <td>
							 	<button class='btn btn-success' onClick='VerificarModificarMotivo(" . $ID . ",\"" . $Fecha . "\",\"" . $Motivo . "\",\"" . $Codigo . "\"," . $Num_Motivo . "," . $ID_Motivo . ")'>
									<i class='fa fa-check'></i>
								</button>
								<button class='btn btn-danger' onClick='CancelarModificacionMotivo(" . $Ret["ID"] . ")'>
									<i class='fa fa-times'></i>
								</button>
							 </td>
						  </tr>";
