...
class ResponsableController 
{
    ...
    public function new_responsable_control()
    {
        ...
        $resp_nombre = ucfirst($_REQUEST["Responsable"]);
        $Estado = 1;

        $fecha = date("Y-m-d");
        $id_tipo_accion = 1;
        $detalles = "El usuario con ID: $id_usuario ha registrado un nuevo Responsable. Datos: nombre - $resp_nombre";
        ...
        try {
            ...
            if(Responsable::get_id_responsable_by_name(coneccion_base: $Con, responsable: $resp_nombre) > 0){
                $Con->CloseConexion();
                $Mensaje = "Ya existe un Responsable con ese Nombre";
    ...
    public function buscar_responsable($valor, $id)
    {
        ...
        if (isset($consulta)) {
            ...
            $list = Responsable::get_list_responsable_por_nombre(coneccion_base: $con, 
                                                                 responsable: $consulta);
            $con->CloseConexion();
            $can = count($list);
            ...
            if (!$can) {
                ...
                foreach($list as $val => $responsable) {
                    $mensaje .= '<tr>
                                    <tr scope="row">' . $responsable->get_id_responsable() . '</tr>
                                    <td>' . $responsable->get_responsable() . '</td>
                                    <td>
                                        <button type = "button" class = "btn btn-outline-success" 
                                                onClick="seleccionResponsable(\'' . $responsable->get_responsable() . '\', ' . $responsable->get_id_responsable() . ', ' . $responsable_nro . ')" 
                                                data-dismiss="modal">
                                            seleccionar
                                        </button>
                                    </td>
